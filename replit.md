...
- **Paper A** (Core Methods): IN ACTIVE SUBMISSION — transferred to PLOS ONE (April 2026). v2.3 May 2026: Limitations section strengthened — Limitation 6 now explicitly names ChIP-seq/KO/KEGG sources for all category assignments (anti-circularity); new Limitation 7 flags within-cohort nature of 12-tissue mouse validation. Cover letter at public/downloads/PaperA_CoverLetter_PLOSONE.pdf. Preprint on Research Square: https://doi.org/10.21203/rs.3.rs-9283100/v1
...
- **Paper F** (Expression Persistence / mRNA Half-Life Independence): NOT in submission. Targeting Genome Biology. Preprint on Research Square: https://doi.org/10.21203/rs.3.rs-9385465/v1. v2 upload in progress.
...
- **Paper P** (Temporal Correlation Length): Draft. Not submitted. Package at paper-packages/paper-p-temporal-correlation/. v1.0 May 2026 (14 pages, 234,025 bytes). Reframes |λ| as temporal correlation length ξ — biological analogue of condensed-matter correlation length. G(τ) = |λ|^τ·cos(πτ/T₀); τ_c = −2/ln|λ| (Convention A: per-tissue category mean). Clock τ_c = 3.9h (12 mouse tissues; 3.8h all 13), target τ_c = 2.0h, ratio 2.0× (95% CI [1.6, 2.4]), 13/13 tissues, 18.5× 24h residual. Tissue validation: ρ=0.794 (gap vs rhythmic genes), ρ=0.822 (gap vs GR), ρ=0.802 (ratio vs rhythmic), ρ=0.789 (ratio vs GR), p≤0.002. Disease phase diagram: healthy 1.74×, Bmal1-KO 0.99×, APC-KO 0.43× (E2F programme vs clock), DblKO rescue 1.22×. Platform page: /temporal-correlation. Targeting PLOS Computational Biology. Submission blockers: author names/ORCID/funding, bioRxiv preprint.
- **Paper Q** (Central-Peripheral Clock Hierarchy / Light Entrainment): Draft. Not submitted. Manuscript at manuscripts/paper_q_light_entrainment.md. **v1.1 May 2026**: baboon cross-species replication added (pre-specified, GSE98965, 60 tissues, direct SCN measurement). Key finding: Hypothalamus (SCN proxy) has LOWEST mean clock gene |λ|=0.469; Lung HIGHEST |λ|=0.797 (1.70× range). Re-entrainment lag ratio Lung/Hyp = 2.84× (τ_c 8.8h vs 3.1h). Replication in GSE11923 liver (48h hourly, mean |λ|=0.735). Baboon replication: SCN |λ|=0.4708 (vs mouse Hyp |λ|=0.4690 — virtually identical across ~30M years evolution); CNS<Peripheral gap p=0.022 (permutation); LUN τc=5.48h vs SCN τc=2.65h (ratio 2.07×); 3/4 pre-specified predictions pass. Supplementary Table S3 (TableS4_Baboon_CrossSpecies_Validation.csv) added to package. Platform page: /light-entrainment. Package: paper-packages/paper-q-light-entrainment/. Targeting Journal of Biological Rhythms. Submission blockers: author names/ORCID/funding, permutation tests (planned), phase-shift validation dataset.
