...
-   **Regulatory Core Discovery**: Pathway-agnostic genome-wide scan identifying genes with clock-like dynamics (top 5% |λ| + complex roots) across 12 tissues without pre-defined categories. Recovers known clock genes as positive controls and surfaces novel candidates. Server: `server/regulatory-core-discovery.ts`. Frontend: `/regulatory-discovery`.
...
-   **GSE179028 Enteroid Datasets**: Independent replication dataset for LGR5 gating. Mouse subseries GSE179027 (12,409 genes, 24 timepoints, 2h intervals) and Human subseries GSE161566 (14,250 genes, 24 timepoints). Integrated as embedded datasets. Source: Rosselot et al. 2022. Data: `datasets/GSE179027_MouseEnteroid_circadian.csv`, `datasets/GSE161566_HumanEnteroid_circadian.csv`. AR(2) results: `datasets/GSE179028/GSE179027_AR2_results.csv`, `datasets/GSE179028/GSE179027_platform_data.json`.
-   **Boman-Style Crypt Simulation**: Discrete-time 3-compartment crypt model (Stem → Proliferating → Differentiated) with Boman-like division rules and Fibonacci recursion via maturation delay. Runs 810+ parameter sweeps across 8 conditions (normal, FAP-like, adenoma-like, high/low Wnt, strong delay feedback, balanced oscillator). Fits AR(2) to each output and classifies patterns (Fibonacci-consistent, Lucas-like, etc.). Generates 4 downloadable CSV files + README ZIP. Server: `server/boman-simulation.ts`. Frontend: `/boman-simulation`. API: `/api/boman-simulation/generate` and `/api/boman-simulation/download/:fileType`.
